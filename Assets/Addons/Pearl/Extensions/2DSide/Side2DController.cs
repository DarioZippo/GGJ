...
namespace Pearl.Side2D
{
    ...
    /// </summary>
    public class Side2DController : MonoBehaviour
    {
        /// the various states of our character
        public Side2DControllerState State { get; protected set; }

        [Header("Default Parameters")]

        /// the initial parameters
        [Tooltip("the initial parameters")]
        public Side2DControllerParameters DefaultParameters;
        /// the current parameters
        public Side2DControllerParameters Parameters { get { return _overrideParameters ?? DefaultParameters; } set { _overrideParameters = value; } }

        [Header("Collisions")]
        [Information("You need to define what layer(s) this character will consider a walkable platform/moving platform etc. By default, you want Platforms, MovingPlatforms, OneWayPlatforms, MovingOneWayPlatforms, in this order.", InformationTypeEnum.Info, false)]

        /// The layer mask the platforms are on
        [Tooltip("The layer mask the platforms are on")]
        public LayerMask PlatformMask = LayerMaskSide2DController.PlatformsLayerMask | LayerMaskSide2DController.PushablesLayerMask;
        /// The layer mask the moving platforms are on
        [Tooltip("The layer mask the moving platforms are on")]
        public LayerMask MovingPlatformMask = LayerMaskSide2DController.MovingPlatformsLayerMask;
        /// The layer mask the one way platforms are on
        [Tooltip("The layer mask the one way platforms are on")]
        public LayerMask OneWayPlatformMask = LayerMaskSide2DController.OneWayPlatformsLayerMask;
        /// The layer mask the moving one way platforms are on
        [Tooltip("The layer mask the moving one way platforms are on")]
        public LayerMask MovingOneWayPlatformMask = LayerMaskSide2DController.MovingOneWayPlatformsMask;
        /// The layer mask the mid height one way paltforms are on
        [Tooltip("The layer mask the mid height one way paltforms are on")]
        public LayerMask MidHeightOneWayPlatformMask = LayerMaskSide2DController.MidHeightOneWayPlatformsLayerMask;
        /// The layer mask the stairs are on
        [Tooltip("The layer mask the stairs are on")]
        public LayerMask StairsMask = LayerMaskSide2DController.StairsLayerMask;
        ...
        [Information("If you set SafeSetTransform to true, abilities that modify directly the character's position, like CharacterGrip, will perform a check to make sure there's enough room to do so." +
                       "This isn't enabled by default because this comes at a (small) performance cost and can usually be avoided by having a safe level design.", InformationTypeEnum.Info, false)]
        ...
        [Header("Raycasting")]
        [Information("Here you can define how many rays are cast horizontally and vertically. " +
            "You'll want them as far as possible from each other, but close enough that no obstacle or enemy can fit between 2 rays.",
            InformationTypeEnum.Info, false)]
        ...
        [Header("Stickiness")]
        [Information("Here you can define whether or not you want your character stick to slopes when walking down them, and how long the raycast handling that should be (0 means automatic length).", InformationTypeEnum.Info, false)]
        ...
        [Header("Safety")]
        [Information("Here you can authorize your controller to start rotated. This will change its gravity direction. It's safer to leave this safety on and use a CharacterGravity ability instead.", InformationTypeEnum.Info, false)]
        ...
        public Vector2 Direction => Speed.normalized;
        ...
        public virtual float DeltaTime => UpdateMode != UpdateModes.FixedUpdate ? Time.deltaTime : Time.fixedDeltaTime;
        public float Friction => _friction;
        public ISurfaceModifier CurrentSurfaceModifier { get; set; }
        public GameObject[] StandingOnArray { get; set; }

        bool _addSpeedX = false;
        bool _addSpeedY = false;
        ...
        protected Side2DControllerParameters _overrideParameters;
        ...
        protected ISurfaceModifier _frictionTest;
        ...
        private Vector2 _impulseForce = Vector2.zero;

        protected List<RaycastHit2D> _contactList;
        protected bool _shouldComputeNewSpeed = false;

        private bool _thereIsSlowFall = false;
        private readonly List<CharacterAbility> _abilitesThatCheckNewPosition = new();
        ...
        public void AddAbilityCheckForNewPosition(CharacterAbility ability)
        {
            _abilitesThatCheckNewPosition?.Add(ability);
        }

        public void RemoveAbilityCheckForNewPosition(CharacterAbility ability)
        {
            _abilitesThatCheckNewPosition?.Remove(ability);
        }

        protected virtual void Initialization()
        {
            // we get the various components
            DefaultParameters?.vectorGravity.Normalize();
            ...
            {
                UnityEngine.Debug.LogWarning("The boxcollider for " + gameObject.name + " should have an x offset set to zero. Right now this may cause issues when you change direction close to a wall.");
            }

            // raycast list and state init
            _contactList = new List<RaycastHit2D>();
            State = new Side2DControllerState();
        ...
        protected virtual void ApplyGravitySettings()
        {
            ...
            {
                _characterGravity = gameObject.GetComponent<CharacterGravity>();
        ...
        #region Force
        private float AddGravity(float y)
        {
            if (Mathf.Sign(Parameters.Gravity) != Mathf.Sign(y))
            {
                return Mathf.Sqrt(2f * y * Mathf.Abs(Parameters.Gravity) / Parameters.AscentMultiplier);
            }
            else
            {
                return y;
        ...
        public virtual void AddForce(Vector2 force, bool useExternalForce = false, bool addGravity = false, bool isImpulseForce = false)
        {
            if (addGravity)
            {
                force.y = AddGravity(force.y);
            }
        ...
        public virtual void AddHorizontalForce(float x, bool useExternalForce = false, bool isImpulseForce = false)
        {
            if (isImpulseForce)
            {
                _impulseForce += Vector2.right * x;
            }
        ...
        public virtual void AddVerticalForce(float y, bool useExternalForce = false, bool addGravity = false, bool isImpulseForce = false)
        {
            if (addGravity)
            {
                y = AddGravity(y);
        ...
        public virtual void SetForce(Vector2 force, bool useExternalForce = false, bool addSpeedX = false, bool addSpeedY = false, bool addGravity = false, bool isImpulseForce = false)
        {
            if (isImpulseForce)
            {
                _impulseForce += force;
            ...
            if (addSpeedX && !_addSpeedX)
            {
                force += Speed.x * Vector2.right;
                _addSpeedX = true;
            }

            if (addSpeedY && !_addSpeedY)
            {
                force += Speed.y * Vector2.up;
                _addSpeedY = true;
            }
        ...
        /// </summary>
        /// <param name="x">The x value of the velocity.</param>
        public virtual void SetHorizontalForce(float x, bool useExternalForce = false, bool isImpulseForce = false)
        {
            if (isImpulseForce)
            {
                _impulseForce += Vector2.right * x;
        ...
        /// </summary>
        /// <param name="y">The y value of the velocity.</param>
        public virtual void SetVerticalForce(float y, bool useExternalForce = false, bool addGravity = false, bool isImpulseForce = false)
        {
            if (addGravity)
            {
                y = AddGravity(y);
        ...
        protected virtual void EveryFrame()
        {
            ...
            _speed -= _impulseForce;
            _impulseForce = Vector2.zero;
            _addSpeedX = false;
            _addSpeedY = false;
        }
        ...
        protected virtual void MoveTransform()
        {
            ...
            {
                _stickRaycast = Physics2DExtend.BoxCast(_boundsCenter, Bounds, Vector2.Angle(transform.up, Vector2.up), _newPosition.normalized, _newPosition.magnitude, PlatformMask, Color.red, true);
                if (_stickRaycast)
                {
            ...
            foreach (var ability in _abilitesThatCheckNewPosition)
            {
                ability.CheckNewPosition(ref _newPosition);
            }
        ...
        protected virtual void ApplyGravity()
        {
            ...
            if (_speed.y > 0)
            {
                _currentGravity /= Parameters.AscentMultiplier;
            }
            if (_speed.y < 0)
            {
                _currentGravity *= Parameters.FallMultiplier;
            }

            float aux = 0;
            if (_gravityActive)
            {
                aux = (_currentGravity + _movingPlatformCurrentGravity) * DeltaTime;
                if (_thereIsSlowFall)
                {
                    aux *= _fallSlowFactor;
                }
            ...
            switch (direction)
            {
                case RaycastDirections.left:
                    ...
                    for (int i = 0; i < NumberOfHorizontalRays; i++)
                    {
                        ...
                        storageArray[i] = Physics2DExtend.RayCast(rayOriginPoint, -transform.right, rayLength, PlatformMask & ~OneWayPlatformMask & ~MovingOneWayPlatformMask, Parameters.DrawRaycastsGizmos, color);
                        if (storageArray[i])
                        {
                            returnValue = true;
                        }
                ...
                case RaycastDirections.right:
                    ...
                    _horizontalRayCastToTop = (_boundsTopLeftCorner + _boundsTopRightCorner) / 2;
                    _horizontalRayCastFromBottom = _horizontalRayCastFromBottom + (Vector2)transform.up * ObstacleHeightTolerance;
                    _horizontalRayCastToTop -= (Vector2)transform.up * ObstacleHeightTolerance;
                    for (int i = 0; i < NumberOfHorizontalRays; i++)
                    {
                        Vector2 rayOriginPoint = Vector2.Lerp(_horizontalRayCastFromBottom, _horizontalRayCastToTop, (float)i / (float)(NumberOfHorizontalRays - 1));
                        storageArray[i] = Physics2DExtend.RayCast(rayOriginPoint, transform.right, rayLength, PlatformMask & ~OneWayPlatformMask & ~MovingOneWayPlatformMask, Parameters.DrawRaycastsGizmos, color);
                ...
                case RaycastDirections.down:
                    ...
                    for (int i = 0; i < NumberOfVerticalRays; i++)
                    {
                        ...
                        {
                            storageArray[i] = Physics2DExtend.RayCast(rayOriginPoint, -transform.up, rayLength, PlatformMask & ~OneWayPlatformMask & ~MovingOneWayPlatformMask, Parameters.DrawRaycastsGizmos, color);
                            if (storageArray[i])
                            {
                                returnValue = true;
                            }
        ...
        protected virtual void CastRaysToTheSides(float raysDirection)
        {
            ...
            for (int i = 0; i < NumberOfHorizontalRays; i++)
            {
                ...
                {
                    _sideHitsStorage[i] = Physics2DExtend.RayCast(rayOriginPoint, raysDirection * (transform.right), horizontalRayLength, PlatformMask, Parameters.DrawRaycastsGizmos, ColorExtend.GetColor(ColorEnum.Indigo));
                }
                else
                {
                    _sideHitsStorage[i] = Physics2DExtend.RayCast(rayOriginPoint, raysDirection * (transform.right), horizontalRayLength, PlatformMask & ~OneWayPlatformMask & ~MovingOneWayPlatformMask, Parameters.DrawRaycastsGizmos, ColorExtend.GetColor(ColorEnum.Indigo));
                }
                ...
                if (_sideHitsStorage[i].distance > 0)
                {
                    ...
                    if (hitAngle > Parameters.MaximumSlopeAngle)
                    {
                        ...
                        if ((_movementDirection == raysDirection) || (CastRaysOnBothSides && (_speed.x == 0f)))
                        {
                            ...
                            float distance = GeometryUtils.DistanceBetweenPointAndLine(_sideHitsStorage[i].point, new Segment(_horizontalRayCastFromBottom, _horizontalRayCastToTop));
                            if (raysDirection <= 0)
                            {
        ...
        protected virtual void CastRaysBelow()
        {
            ...
            for (int i = 0; i < NumberOfVerticalRays; i++)
            {
                ...
                if ((_newPosition.y > 0) && (!State.WasGroundedLastFrame))
                {
                    _belowHitsStorage[i] = Physics2DExtend.RayCast(rayOriginPoint, -transform.up, rayLength, _raysBelowLayerMaskPlatformsWithoutOneWay, Parameters.DrawRaycastsGizmos, Color.blue);
                }
                else
                {
                    _belowHitsStorage[i] = Physics2DExtend.RayCast(rayOriginPoint, -transform.up, rayLength, _raysBelowLayerMaskPlatforms, Parameters.DrawRaycastsGizmos, Color.blue);
                }

                float distance = GeometryUtils.DistanceBetweenPointAndLine(_belowHitsStorage[smallestDistanceIndex].point, new Segment(_verticalRayCastFromLeft, _verticalRayCastToRight));
                ...
                if (_belowHitsStorage[i])
                {
                    ...
                    State.BelowSlopeAngleAbsolute = MathfExtend.AngleBetween(_belowHitsStorage[i].normal, Vector2.up);
            ...
            if (hitConnected)
            {
                ...
                if (
                    ...
                    && (
                        ...
                        ||
                        (MovingOneWayPlatformMask.Contains(StandingOn.layer) && (_speed.y > 0))
                    )
                )
                {
                    StandingOn = null;
        ...
        protected virtual void CastRaysAbove()
        {
            ...
            for (int i = 0; i < NumberOfVerticalRays; i++)
            {
                ...
                _aboveHitsStorage[i] = Physics2DExtend.RayCast(rayOriginPoint, (transform.up), rayLength, PlatformMask & ~OneWayPlatformMask & ~MovingOneWayPlatformMask, Parameters.DrawRaycastsGizmos, ColorExtend.GetColor(ColorEnum.Cyan));
        ...
        protected virtual void StickToSlope()
        {
                ...
                //|| (_movingPlatform != null))
            {
                ...
                if (!(!State.WasGroundedLastFrame
                      && ((LastStandingOn != null) && StairsMask.Contains(LastStandingOn.layer))
                      && !(State.IsJumping)
                    ))
                {
                    return;
            ...
            _stickRaycastLeft = Physics2DExtend.RayCast(_rayCastOrigin, -transform.up, rayLength, _raysBelowLayerMaskPlatforms, Parameters.DrawRaycastsGizmos, ColorExtend.GetColor(ColorEnum.LightBlue));

            _rayCastOrigin.x = _boundsBottomRightCorner.x;
            _rayCastOrigin.x += _newPosition.x;
            _stickRaycastRight = Physics2DExtend.RayCast(_rayCastOrigin, -transform.up, rayLength, _raysBelowLayerMaskPlatforms, Parameters.DrawRaycastsGizmos, ColorExtend.GetColor(ColorEnum.LightBlue));
            ...
            if ((belowSlopeAngleLeft > 0f) && (belowSlopeAngleRight < 0f))
            {
                _stickRaycast = Physics2DExtend.BoxCast(_boundsCenter, Bounds, Vector2.Angle(transform.up, Vector2.up), -transform.up, rayLength, _raysBelowLayerMaskPlatforms, Color.red, true);
                if (_stickRaycast)
                {
        ...
        protected virtual void ComputeDistanceToTheGround()
        {
            ...
            _distanceToTheGroundRaycast = Physics2DExtend.RayCast(_rayCastOrigin, -transform.up, DistanceToTheGroundRayMaximumLength, _raysBelowLayerMaskPlatforms, true, ColorExtend.GetColor(ColorEnum.CadetBlue));
        ...
        public virtual void SlowFall(float factor, bool thereIsSlowFall)
        {
            _fallSlowFactor = factor;
            this._thereIsSlowFall = thereIsSlowFall;

            if (factor == 0 && thereIsSlowFall)
            {
                _speed.y = 0;
            }
        }
        ...
        public virtual bool CanGoBackToOriginalSize()
        {
            ...
            bool headCheckLeft = Physics2DExtend.RayCast(_originalSizeRaycastOrigin, transform.up, headCheckDistance, PlatformMask - OneWayPlatformMask, true, ColorExtend.GetColor(ColorEnum.LightSlateGray));

            _originalSizeRaycastOrigin = _boundsTopRightCorner + (Vector2)transform.up * _smallValue;
            bool headCheckRight = Physics2DExtend.RayCast(_originalSizeRaycastOrigin, transform.up, headCheckDistance, PlatformMask - OneWayPlatformMask, true, ColorExtend.GetColor(ColorEnum.LightSlateGray));
        ...
        /// </summary>
        protected virtual void OnSideColliderHit()
        {
            foreach (RaycastHit2D hit in _contactList)
            {
                if (Parameters.Physics2DInteraction)
                {
                    ...
                    Vector3 pushDirection = new(_externalForce.x, 0, 0);
