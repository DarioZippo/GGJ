...
namespace Pearl.Side2D
{
    ...
    public class CharacterGravity : CharacterAbility
    {
        ...
        [Information("Here you can decide whether or not this character is subject to Gravity Points and Gravity Zones, or if it should ignore them.", InformationTypeEnum.Info, false)]
        ...
        [Information("Here you can define the initial gravity angle, and whether or not horizontal and/or vertical input should be reversed when upside down.", InformationTypeEnum.Info, false)]
        ...
        [Information("Here you can set a rotation speed that will be applied to your character when changing zones. A speed of 0 means instant rotation. " +
            "Be careful with that setting, as it'll gradually change the gravity direction of your character, which may not be what you want. You can also specify " +
            "the Inactive Buffer Duration, which is the time during which a zone is disabled on entry/exit to accomodate for the rotation's duration. As a general rule," +
            " slow speeds will require higher inactive buffer duration, so the lower the speed, the higher the inactive buffer duration should be. A rotation speed of 1.6 and " +
            "a buffer duration of 0.3 usually works well, but feel free to play with these settings.", InformationTypeEnum.Info, false)]
        ...
        [Information("Here you can define a transition mode that will impact the way your character enters or exits gravity altered zones. Reset will kill all current movement " +
            "and state, which is the safest option. Adapt will keep your force going and rotate it, and Nothing will not affect existing forces, which may have weird consequences " +
            "depending on the context.", InformationTypeEnum.Info, false)]
        ...
        [Information("Here you can chose to have the Ability draw the current gravity target angle.", InformationTypeEnum.Info, false)]
        ...
        public Vector2 GravityDirectionVector { get { return Vector2Extend.RotateVector2(Vector2.down, GravityAngle); } }
        ...
        protected virtual void ComputeGravityPoints()
        {
            ...
            if (_closestGravityPoint != null)
            {
                ...
                if ((_lastGravityPoint != _closestGravityPoint) && (_lastGravityPoint != null))
                {
                    ...
                    if (Time.time - _entryTimeStampPoints >= InactiveBufferDuration)
                    {
                        ...
                        Transition(true, _newGravityPoint.transform.position - controller.ColliderCenterPosition);
                        StartRotating();
                    }
                }
                ...
                _gravityPointDirection = _newGravityPoint.transform.position - controller.ColliderCenterPosition;
                float gravityAngle = 180 - Vector2Extend.AngleBetween(Vector2.up, _gravityPointDirection);
            ...
            else
            {
                ...
                if (Time.time - _entryTimeStampPoints >= InactiveBufferDuration)
                {
                    if (_lastGravityPoint != null)
                    {
                        Transition(false, _newGravityPoint.transform.position - controller.ColliderCenterPosition);
                        StartRotating();
                    }
                    _entryTimeStampPoints = Time.time;
        ...
        protected virtual GravityPoint GetClosestGravityPoint()
        {
            ...
            Vector3 currentPosition = controller.ColliderCenterPosition;
        ...
        public virtual void UpdateGravityPointsList()
        {
            ...
            _gravityPoints.AddRange(Object.FindObjectsByType<GravityPoint>(FindObjectsSortMode.None));
        }
        ...
        protected virtual void OnTriggerEnter2D(Collider2D collider)
        {
            ...
            */
            ...
            GravityZone gravityZone = collider.gameObject.GetComponent<GravityZone>();
        ...
        {
            _rotationDirection = manager.CurrentFacing == SemiAxisX.Right ? 1 : -1;
        }
        ...
        {
            float gravityAngle = 180 - Vector2Extend.AngleBetween(Vector2.up, gravityDirection);
            ...
            if (TransitionForcesMode == TransitionForcesModes.Adapt)
            {
                ...
                controller.SetForce(Quaternion.Euler(0, 0, rotationAngle) * controller.Speed);
            }
