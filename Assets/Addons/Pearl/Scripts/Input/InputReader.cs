...
namespace Pearl
{
    public class InputReader : PearlBehaviour
    {
        ...
        public class InputInfo
        {
            ...
            public string mapAction = string.Empty;
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < _inputsNoVoid.Count; i++)
            {
                ...
                {
                    var valueFloat = inputInterface.GetAxis(inputInfo.nameAction, inputInfo.raw, inputInfo.mapAction);
                ...
                {
                    var valueVector = inputInterface.GetVectorAxis(inputInfo.nameAction, inputInfo.raw, inputInfo.mapAction);
        ...
        private void AnalizeVoidInput(ActionEvent actionEvent)
        {
            for (int i = 0; i < inputInfos.Length; i++)
            {
                ...
                if (inputInfo.output == InputType.Button)
                {
                    ...
                    inputInterface.PerformedHandle(inputInfo.nameAction, inputInfo.auxAction, actionEvent, inputInfo.stateButton, inputInfo.mapAction, inputInfo.order);
                    inputInterface.ChangeInterrupt(inputInfo.interrupt, inputInfo.nameAction, inputInfo.stateButton, inputInfo.mapAction);
                }
            }
        }
        #endregion
