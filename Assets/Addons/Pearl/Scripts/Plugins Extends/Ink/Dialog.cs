...
namespace Pearl
{
    public struct TextStruct
    {
        ...
        public TextStruct(string title, string text, string clipID)
        {
            this.title = title;
            this.text = text;
            this.clipID = clipID;
    ...
    public class Dialog : IReset, IPause
    {
        ...
        private void ReadStartTrigger()
        {
            ...
            if (!_tags.TryGetValue(audioTag, out _clipString))
            {
                _clipString = string.Empty;
            }
        ...
        {
            TextStruct textStruct = new(_title, _currentPiece.text, _clipString);
