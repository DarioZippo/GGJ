...
namespace Pearl.Storage
{
    ...
    public static class StorageManager
    {
        ...
        #region Path
        private static string GetRelativePath(string relativePath, object behaviour, Type type, bool isGetter)
        {
            if (isGetter)
            {
                return behaviour != null ? ReflectionExtend.Getter<String>(behaviour, relativePath) :
                    ReflectionExtend.Getter<String>((object)type, relativePath);
            }
            else
            {
                return relativePath;
    ...
    catch(Exception e) 
    {
            ...
            if (members.IsAlmostSpecificCount())
            {
                ...
                foreach (var member in members)
                {
                    if (member.GetCustomAttribute(typeof(StorageAttribute), false) is StorageAttribute attribute)
                    {
                        ...
                        string key = ID + member.Name + GetRelativePath(attribute.RelativePath, behaviour, currentType, attribute.IsGetter);
                        var value = member.Getter(currentContainer);
                        var nameVar = IDForVar + member.Name;
                        MemberComplexInfo memberComplexInfo = new(member, currentContainer);

                        string completePath = GetCompletePath(GetRelativePath(attribute.RelativePath, behaviour, currentType, attribute.IsGetter), attribute.AbsolutePath, attribute.IsSlot);
        ...
        {
            elementStorage.memberComplexInfo.SetValue(elementStorage.value);
        }
        ...
        private static void SaveLight(ElementStorage element)
        {
            ...
            element.value = memberInfo.Getter(container);
        }
        #endregion
        ...
        private static List<MemberInfo> GetStorageAttributes(Type type, string specificMember = null)
        {
            ...
            while (type != null)
            {
                MemberInfo[] membersInfo = string.IsNullOrWhiteSpace(specificMember) ?
                    ...
                    type.GetCustomAttributes<StorageAttribute>(specificMember);
