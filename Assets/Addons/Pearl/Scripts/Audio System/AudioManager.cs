...
namespace Pearl.Audio
{
    ...
    public class AudioManager : PearlBehaviour, ISingleton, IStoragePlayerPrefs
    {
        ...
        protected override void OnApplicationInterrupt(bool hasFocus)
        {
            if (musicSource)
            {
        ...
        #region Music region
        public static void ChangeSpeedMusic(float newSpeed)
        {
            if (GetIstance(out var audioManager))
            {
        ...
        public static void ChangeVolumeMusic(float volume, float timeTranslation)
        {
            if (GetIstance(out var audioManager))
            {
                audioManager.ChangeVolumeInternal(volume, timeTranslation);
            }
        }

        private void ChangeVolumeInternal(float volume, float timeTranslation)
        {
            if (musicSource)
            {
                musicSource.SetVolume(volume, timeTranslation);
            }
        }
