...
namespace Pearl
{
    ...
    public class SpriteManager
    {
        ...
        {
            return obj != null &&
                (obj.TryGetChildInHierarchy<Image>(out var _) || obj.TryGetChildInHierarchy<SpriteRenderer>(out var _) || obj.TryGetChildInHierarchy<TMP_Text>(out var _));
        }
        ...
        public SpriteManager(GameObject owner)
        {
            if (owner)
            {
                ...
                if (owner.TryGetChildInHierarchy<Image>(out var image))
                {
                ...
                if (owner.TryGetChildInHierarchy<SpriteRenderer>(out var renderer))
                {
                    spriteRender = renderer;
                    return;
                }

                if (owner.TryGetChildInHierarchy<TMP_Text>(out var tmp))
                {
                    textRender = tmp;
                    return;
                }
