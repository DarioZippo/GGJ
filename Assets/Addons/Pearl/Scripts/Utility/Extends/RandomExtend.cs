...
namespace Pearl
{
    ...
    public static class RandomExtend
    {
        ...
        public static ICollection<T> GetRandomCollection<T>(this ICollection<T> @this, int numberElements, params T[] exception) where T : new()
        {
            if (@this == null || numberElements >= @this.Count - 1)
            {
                return @this.ToList();
            }
            ...
            var randomRange = new CollectionRandomRange<T>(@this);
            ...
            List<T> result = new();
            numberElements = Mathf.Clamp(numberElements, 0, @this.Count);
            if (randomRange != null)
            {
