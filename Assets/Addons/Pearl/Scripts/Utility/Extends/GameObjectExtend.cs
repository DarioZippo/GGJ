...
namespace Pearl
{
    ...
    public static class GameObjectExtend
    {
        ...
        public static T[] FindOtherObjectsOfType<T>(this GameObject @this) where T : Component
        {
            if (@this != null)
            {
                List<T> list = new(GameObject.FindObjectsByType<T>(FindObjectsSortMode.None));
                if (list != null)
                {
        ...
        public static GameObject Find(StringTypeControl tagType, string[] keys, bool onlyThoseTags = false)
        {
            ...
            {
                var auxes = UnityEngine.Object.FindObjectsByType<GameObject>(FindObjectsSortMode.None);
        ...
        public static T FindInHierarchy<T>(in string rootString) where T : Component
        {
            ...
            if (rootObj)
            {
                return TransformExtend.GetComponent<T>(rootObj);
            }
            return null;
        }

        public static T[] FindAllInterfaces<T>(in FindObjectsInactive includeInactive = FindObjectsInactive.Exclude)
        {
            List<T> interfaces = new();
            GameObject[] rootGameObjects = GameObject.FindObjectsByType<GameObject>(includeInactive, FindObjectsSortMode.None);
        ...
        #region Destroy
        public static void FakeDestroy(this GameObject @this, bool destroy)
        {
            if (@this != null)
            {
                var pearlObject = @this.AddOnlyOneComponent<PearlObject>();
                if (pearlObject)
                {
                    pearlObject.FakeDestroy(destroy);
                }
            }
        }

        public static bool IsFakeDestroyed(this GameObject @this)
        {
            if (@this != null && @this.TryGetComponent<PearlObject>(out var pearlObject))
            {
                return pearlObject.FakeDestroyed;
            }
        ...
        public static GameObject CreateGameObject(string name, Transform parent = null, WorldReference parentReference = WorldReference.World, bool worldPositionStays = true, bool dontDestroyAtLoad = false, params Type[] components)
        {
            ...
            if (obj)
            {
                ...
                ParentStruct parentStruct = new(parent, parentReference, worldPositionStays);
        ...
        public static T CreateGameObject<T>(string name, Transform parent = null, WorldReference parentReference = WorldReference.World, bool worldPositionStays = true, bool dontDestroyAtLoad = false, params Type[] components) where T : Component
        {
            GameObject obj = CreateGameObject(name, parent, parentReference, worldPositionStays, dontDestroyAtLoad, components);
            return obj != null ? obj.AddOnlyOneComponent<T>() : default;
        }
        #endregion

        #region Create GameObject With prefab
        public static bool CreateGameObject<T>(GameObject prefab, out T reference, string name = null, Vector3 position = default, Quaternion rotation = default, Transform parent = null, WorldReference parentReference = WorldReference.World, bool worldPositionStays = true, bool onlyInTheScene = false, bool dontDestroyAtLoad = false) where T : Component
        {
            CreateGameObject(prefab, out GameObject obj, name, position, rotation, parent, parentReference, worldPositionStays, onlyInTheScene, dontDestroyAtLoad);
            reference = null;
            if (obj != null)
            {
        ...
        public static bool CreateGameObject(GameObject prefab, out GameObject reference, string name = null, Vector3 position = default, Quaternion rotation = default, Transform parent = null, WorldReference parentReference = WorldReference.World, bool worldPositionStays = true, bool onlyInTheScene = false, bool dontDestroyAtLoad = false)
        {
            if (prefab)
            {
                if (onlyInTheScene)
