...
namespace Pearl.UI
{
    public class UI_MagneticInfiniteScroll : PearlInfiniteScroll
    {
        ...
        [SerializeField, Tooltip("A boolean that indicate if the scroll need initialization with specific index")]
        private bool useInitIndex = false;

        [SerializeField, Tooltip("The pointer to the pivot, the visual element for centering objects.")]
        private RectTransform pivot = null;

        [SerializeField, Tooltip("The pointer to the object container")]
        private RectTransform content = null;

        [SerializeField, Tooltip("the maximum speed that allows you to activate the magnet to center on the pivot")]
        private float maxSpeedForMagnetic = 10f;

        [SerializeField, Tooltip("The initial index of the object which must be initially centered")]
        private int indexStart = 0;

        [SerializeField, Tooltip("The time to decelerate and aim for the pivot")]
        private float timeForDeceleration = 0.05f;

        [SerializeField]
        private UnityEvent eventButton;
        ...
        {
            if (pivot != null && pivot.IsPointerOver() && !_isDrag)
            {
                eventButton?.Invoke();
            }
        }
